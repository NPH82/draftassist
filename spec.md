...
- **Discrepancy email diagnostics logging**: backend now logs safe SMTP diagnostics for devy discrepancy notifications (attempt start/end, timeout wrapper timing, report ID correlation, and provider error metadata without exposing SMTP credentials) to speed production troubleshooting
- **Discrepancy email header hardening**: SMTP `to`/`from` and dynamic subject segments now strip CR/LF and normalize whitespace before send to prevent header-injection attempts from user-supplied values
- **SMTP IPv4 enforcement hardening**: discrepancy email delivery now pre-resolves SMTP hostnames and connects directly to the resolved IPv4 address (while preserving TLS SNI via original hostname) to avoid IPv6 `ENETUNREACH` failures on restricted hosts
- **Devy graduation filter fix**: available devy pool now checks `devyClass <= currentYear` in addition to `nflDraftYear`; scraper-sourced records (KTC/NFLMDB/Sheet) store draft year in `devyClass` only, so 2026-class players were bypassing the graduation gate entirely
- **Devy 2026-class graduation nuance**: current-year `devyClass` players are only excluded when they also have `team` set in the DB (confirmed NFL pick); undrafted or school-returned players stay in the devy pool; past classes (`devyClass < currentYear`) are always swept
- **refreshDevyRankings graduation sweep**: after each rankings refresh a bulk `updateMany` now clears `isDevy` from any record where `devyClass <= currentYear` so the DB self-heals on each scheduled run
- **fix-devy-flags no-sleeperId path**: admin endpoint no longer blindly keeps scraper-sourced players that lack a Sleeper ID; now checks `devyClass`/`nflDraftYear` and clears graduated players even when no Sleeper record is present
